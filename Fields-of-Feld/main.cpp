...
{
	Character player;;
	Item ironArrow = ironArrow.createAmmunition("Iron Arrow", "A simple iron arrow", 25, 1, 0.1, 10, 20, Item::physical_damage_types::PIERCE, Item::ARROW);
	Item steelArrow = steelArrow.createAmmunition("Steel Arrow", "A well-crafted steel arrow", 35, 2, 0.1, 10, 20, Item::physical_damage_types::PIERCE, Item::ARROW);
	player.inventory.addAmmunition(ironArrow);
	player.inventory.addAmmunition(steelArrow);
	player.inventory.equipItem(player.inventory.findAmmunition(ironArrow.name), Item::QUIVER1);
	player.inventory.equipItem(player.inventory.findAmmunition(steelArrow.name), Item::QUIVER2);
...
void printMainMenu(Character character)
{
	...
	Character wizard;
	wizard.setCharacterClass(Character::CharacterClass::WIZARD);
	Character battleMage;
	battleMage.setCharacterClass(Character::CharacterClass::BATTLEMAGE);
	Character knight;
	knight.setCharacterClass(Character::CharacterClass::KNIGHT);
	Character cleric;
	cleric.setCharacterClass(Character::CharacterClass::CLERIC);
	Character hunter;
	hunter.setCharacterClass(Character::CharacterClass::HUNTER);
	Character highlander;
	highlander.setCharacterClass(Character::CharacterClass::HIGHLANDER);
	Character wretch;
	wretch.setCharacterClass(Character::CharacterClass::WRETCH);
	...
	do
	{
		cout << dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl
			...
			<< dye::light_yellow(" |   1) Continue     |   |   2) New Game     |   | 3) View Inventory |   | 4) View Character |   |      5) Exit      |  ") << endl
			<< dye::light_yellow(" =-------------------=   =-------------------=   =-------------------=   =-------------------=   =-------------------=  ") << endl
			<< dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl;
		...
		case 1:
		{
			...
			{
				Character easyEnemy1;
				easyEnemy1.namedCharacter = false;
				easyEnemy1.isAlive = true;
				easyEnemy1.health = 10;
				easyEnemy1.mana = 5;
				easyEnemy1.strength = 10;
				easyEnemy1.agility = 15;
				easyEnemy1.luck = 2;
				easyEnemy1.level = 4;
				easyEnemy1.name = "Lowly Bandit";
				easyEnemy1.distanceFromPlayer = 51;
				easyEnemy1.gold = 50;
				easyEnemy1.confidenceLevel = 100;

				Character easyEnemy2;
				easyEnemy2.namedCharacter = false;
				easyEnemy2.isAlive = true;
				easyEnemy2.health = 16;
				easyEnemy2.mana = 5;
				easyEnemy2.strength = 10;
				easyEnemy2.agility = 21;
				easyEnemy2.luck = 2;
				easyEnemy2.level = 7;
				easyEnemy2.name = "Bandit";
				easyEnemy2.distanceFromPlayer = 50;
				easyEnemy2.gold = 75;
				easyEnemy2.confidenceLevel = 100;

				Character easyEnemy3;
				easyEnemy3.namedCharacter = true;
				easyEnemy3.isAlive = true;
				easyEnemy3.health = 19;
				easyEnemy3.mana = 5;
				easyEnemy3.strength = 16;
				easyEnemy3.agility = 24;
				easyEnemy3.luck = 2;
				easyEnemy3.level = 11;
				easyEnemy3.name = "Ragnar the Red";
				easyEnemy3.distanceFromPlayer = 50;
				easyEnemy3.gold = 75;
				easyEnemy3.confidenceLevel = 100;

				//behold, the shitbrick TM
				...
				easyEnemy1.inventory.equipItem(ironDagger, Item::MAINHAND1);
				...
				Item armor = armor.createArmor("Rusty Iron Chainmail", "DESC", Item::CHAIN, 30, 10, 15);
				easyEnemy1.inventory.equipItem(armor, Item::equip_slots::CHEST);
				Item shield = shield.createWeapon("Banded Iron Shield", "DESC", 5, 2, 5, 5, 15, false, false, Item::weapon_types::MEDIUMSHIELD, Item::physical_damage_types::BLUNT, Item::magic_damage_types::NOMAGICDAMAGE);
				easyEnemy1.inventory.equipItem(shield, Item::equip_slots::OFFHAND1);
				Item talisman = talisman.createTool("Tattered Cloth Talisman", "A small piece of cloth with a holy symbol stitched into it", Item::HOLYTOOL, Item::TALISMAN, 1, 20, 1, 20);
				easyEnemy1.inventory.equipItem(talisman, Item::OFFHAND2);
				Item amulet = amulet.createJewelry("Rusty Iron Amulet", "DESC", Item::item_types::TRINKET, 10, 1, 1);
				easyEnemy1.inventory.equipItem(amulet, Item::equip_slots::AMULET);
				easyEnemy1.inventory.addItemToBackpack(bow);
				easyEnemy1.inventory.addLootItems(locket);
				easyEnemy1.inventory.addPotion(potion);
				Item halberd = halberd.createWeapon("Halberd", "A long polearm with an axe blade and a spear tip", 15, 15, 40, 10, 20, true, false, Item::weapon_types::HALBERD, Item::physical_damage_types::PIERCE, Item::magic_damage_types::NOMAGICDAMAGE);
				easyEnemy3.inventory.equipItem(halberd, Item::MAINHAND1);
				Item goldRing = goldRing.createJewelry("Gold Ring", "DESC", Item::item_types::TRINKET, 50, 1, 1);
				easyEnemy3.inventory.equipItem(goldRing, Item::equip_slots::RING1);
				character.inventory.addPotion(potion);
				character.inventory.addPotion(biggerPotion);
				vector<Item> items = { ironDagger };
				easyEnemy1.refreshCharacterStats();
				easyEnemy1.setArmorValues(items);
				easyEnemy2.inventory.equipItem(ironDagger, Item::MAINHAND1);
				easyEnemy2.inventory.equipItem(armor, Item::equip_slots::CHEST);
				easyEnemy2.inventory.equipItem(shield, Item::equip_slots::OFFHAND1);
				easyEnemy2.inventory.equipItem(talisman, Item::OFFHAND2);
				easyEnemy2.inventory.addItemToBackpack(bow);
				easyEnemy2.inventory.addLootItems(locket);
				easyEnemy2.inventory.addPotion(potion);
				easyEnemy2.refreshCharacterStats();
				easyEnemy2.setArmorValues(items);
				easyEnemy3.refreshCharacterStats();
				easyEnemy3.setArmorValues(items);

				Creature lootBug = Creature("Loot Bug", 10, 10);
				vector<Character*> enemies;
				enemies.push_back(&easyEnemy1);
				enemies.push_back(&easyEnemy2);
				enemies.push_back(&easyEnemy3);
				enemies.push_back(&lootBug);

				std::optional<std::vector<Character*>> allies;

				vector<Dialogue> dialogues;
				setupDialogue(dialogues, &character, &easyEnemy1);

				//dialogues[0].play();
				openCombat(character, allies, enemies);
				break;
		...
		case 2:
		{
			...
			cout << dye::light_yellow(" =-------------------=") << endl
				<< dye::light_yellow(" |  6) The Wizard    |") << endl
				<< dye::light_yellow(" =-------------------=") << endl;
			...
			cout << dye::light_yellow(" =-------------------=") << endl
				<< dye::light_yellow(" | 5) The Battlemage |") << endl
				<< dye::light_yellow(" =-------------------=") << endl;
			battleMage.printCharacterStats(battleMage);
			cout << dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl;

			cout << dye::light_yellow(" =-------------------=") << endl
				<< dye::light_yellow(" |   4) The Knight   |") << endl
				<< dye::light_yellow(" =-------------------=") << endl;
			knight.printCharacterStats(knight);
			cout << dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl;

			cout << dye::light_yellow(" =-------------------=") << endl
				<< dye::light_yellow(" |   3) The Cleric   |") << endl
				<< dye::light_yellow(" =-------------------=") << endl;
			cleric.printCharacterStats(cleric);
			cout << dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl;

			cout << dye::light_yellow(" =-------------------=") << endl
				<< dye::light_yellow(" |   2) The Hunter   |") << endl
				<< dye::light_yellow(" =-------------------=") << endl;
			hunter.printCharacterStats(hunter);
			cout << dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl;

			cout << dye::light_yellow(" =-------------------=") << endl
				<< dye::light_yellow(" | 1) The Highlander |") << endl
				<< dye::light_yellow(" =-------------------=") << endl;
			highlander.printCharacterStats(highlander);

			cout << dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl
				<< dye::light_yellow(" =-------------------=---=-------------------=---=-------------------=---=-------------------=---=-------------------=  ") << endl
				<< dye::light_yellow(" |                                                 Select Your Class                                                 |  ") << endl
				<< dye::light_yellow(" =-------------------=---=-------------------=---=-------------------=---=-------------------=---=-------------------=  ") << endl
				<< dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl;
			...
			{
			case 1:
				character.setCharacterClass(Character::CharacterClass::HIGHLANDER);
				break;
			case 2:
				character.setCharacterClass(Character::CharacterClass::HUNTER);
				break;
			case 3:
				character.setCharacterClass(Character::CharacterClass::CLERIC);
				break;
			case 4:
				character.setCharacterClass(Character::CharacterClass::KNIGHT);
				break;
			case 5:
				character.setCharacterClass(Character::CharacterClass::BATTLEMAGE);
				break;
			case 6:
				character.setCharacterClass(Character::CharacterClass::WIZARD);
				break;
			case 7:
				character.setCharacterClass(Character::CharacterClass::WRETCH);
				break;
			case 8:
		...
		case 3:
			if (character.active == true)
			{
				cout << dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl
					<< dye::light_yellow(" =----------------------------=              =---------------------------=              =----------------------------=  ") << endl
					<< dye::light_yellow(" |   1) View Equipped Items   |              |   2) View Backpack Items  |              |         3) Go back         |  ") << endl
					<< dye::light_yellow(" =----------------------------=              =---------------------------=              =----------------------------=  ") << endl
					<< dye::light_yellow(" ---------------------------------------------------------------------------------------------------------------------  ") << endl;
...
string getConfidenceDescription(Character character)
{
...
void openCombat(Character& player, optional<vector<Character*>>& allies, vector<Character*>& enemies)
{
	...
	do
	{
		...
		vector<Item*> consumables = player.inventory.getConsumables();
		...
		vector<Potion> healingPotions;
		vector<Potion> manaPotions;
		...
		for (int i = 0; i < player.inventory.potions.size(); i++)
		{
			...
			{
				healingPotions.push_back(player.inventory.potions[i]);
				numPotionCategories++;
			}
			if (player.inventory.potions[i].effects == Potion::MANA)
			{
				manaPotions.push_back(player.inventory.potions[i]);
				numPotionCategories++;
			}
		}
		...
		do
		{
			...
			case 7: // CHECK ENEMY
			{
				...
				for (int i = 0; i < livingEnemiesPointers.size(); i++)
				{
					string confidenceDescription = getConfidenceDescription(*livingEnemiesPointers[i]);
...
void getCombatBanter(Character sender, Character& reciever)
{
	...
	std::vector<Potion> senderPotions = sender.inventory.potions;
	...
	{
		if (senderItems[i].slot == Item::MAINHAND1)
		{
