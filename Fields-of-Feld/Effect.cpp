	...
	for (const auto& effect : target->effects)
	{
		...
		else
		{
			...
			if (effect->name == "Strength")
			{
				...
				if (duration == 0)
				{
					if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->strength -= effect->magnitude;
					effect->removeEffect(target, *effect);
				}
			}
			if (effect->name == "Agility")
			{
				...
				if (duration == 0)
				{
					if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->agility -= effect->magnitude;
					effect->removeEffect(target, *effect);
				}
			}
			if (effect->name == "Intelligence")
			{
				...
				if (duration == 0)
				{
					if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->intelligence -= effect->magnitude;
					effect->removeEffect(target, *effect);
				}
			}
			if (effect->name == "Faith")
			{
				...
				if (duration == 0)
				{
					if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->faith -= effect->magnitude;
					effect->removeEffect(target, *effect);
				}
			}
			if (effect->name == "Arcane")
			{
				...
				if (duration == 0)
				{
					if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->arcane -= effect->magnitude;
					effect->removeEffect(target, *effect);
				}
			}
			if (effect->name == "Charisma")
			{
				...
				if (duration == 0)
				{
					if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->charisma -= effect->magnitude;
					effect->removeEffect(target, *effect);
				}
			}
			if (effect->name == "Luck")
			{
				...
				if (duration == 0)
				{
					if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->luck -= effect->magnitude;
					effect->removeEffect(target, *effect);
				}
			}
	...
	if (effect->name == "Shocking Thorns")
	{
		if (attacker->position[wielder->getId()] <= effect->range)
		{
			...
			if (attacker->namedCharacter) std::cout << dye::light_yellow(" " + attacker->name) << " is" << dye::light_purple(" SHOCKED") <<
				" from an enchantment on " << wielder->name << "'s armor and takes " << effect->getEffectDamage(attacker, effect) << " points of damage!" << std::endl;
			if (!attacker->namedCharacter) std::cout << " The " << attacker->name << " is" << dye::light_purple(" SHOCKED") <<
				" from an enchantment on " << wielder->name << "'s armor and takes " << effect->getEffectDamage(attacker, effect) << " points of damage!" << std::endl;
...
void Effect::applyPassiveEffect(std::shared_ptr<Effect> effect, std::shared_ptr<Character> target)
{
	...
	if (effect->name == "Summon Vulture" && effect->applied == false)
	{
		effect->applied = true;
		Inventory inventory;
		inventory.addToBackpack(std::make_shared<LootItem> (true, "Vulture Feather", "A feather from a vulture", 30.0f, 0.1f, 3.0f, Item::EquipSlots::BACKPACK));
		std::cout << " A large vulture comes to aid " << dye::light_yellow(target->name) << " in combat!" << std::endl;
		std::shared_ptr<Creature> vulture = std::make_shared<Creature>(false, true, true, true, false, "Vulture", "A large vulture", 100, 20, 100, 200, 200, 200, 5, 20, 0, 200, 200, 200, 2000,
			200, 200, 200, 200, 200, 200, 200, 200, inventory, 5, Character::CombatFlags::NEUTRAL);
		target->allies.push_back(vulture);
		effect->removeEffect(target, *effect);
		return;
	}
	if (effect->name == "Summon Wolf" && effect->applied == false)
	{
		effect->applied = true;
		Inventory inventory;
		inventory.addToBackpack(std::make_shared<LootItem>(true, "Wolf Pelt", "A pelt from a wolf", 150.0f, 0.5f, 1.0f, Item::EquipSlots::BACKPACK));
		std::cout << " A rugged wolf comes to aid " << dye::light_yellow(target->name) << " in combat!" << std::endl;
		target->allies.push_back(std::make_shared<Creature>(false, true, true, true, false, "Wolf", "A rugged wolf", 150, 20, 100, 300, 300, 250, 5, 20, 0, 200, 200, 200, 2000,
			200, 200, 200, 200, 200, 200, 200, 200, inventory, 5, Character::CombatFlags::NEUTRAL));
		effect->removeEffect(target, *effect);
		return;
	}
	if (effect->name == "Summon Bear" && effect->applied == false)
	{
		effect->applied = true;
		Inventory inventory;
		inventory.addToBackpack(std::make_shared<LootItem>(true, "Bear Pelt", "A pelt from a bear", 300.0f, 5.0f, 1.0f, Item::EquipSlots::BACKPACK));
		std::cout << " A mountain bear comes to aid " << dye::light_yellow(target->name) << " in combat!" << std::endl;
		target->allies.push_back(std::make_shared<Creature>(false, true, true, true, false, "Bear", "A mountain bear", 250, 20, 100, 400, 400, 200, 5, 20, 0, 200, 200, 200, 2000,
			200, 200, 200, 200, 200, 200, 200, 200, inventory, 5, Character::CombatFlags::NEUTRAL));
		effect->removeEffect(target, *effect);
		return;
	}
...
void Effect::applyConsumableEffect(std::shared_ptr<Effect> effect, std::shared_ptr<Character> target)
{
	...
	if (effect->name == "Random V")
	{
		if (std::dynamic_pointer_cast<std::shared_ptr<Human>>(target)) std::dynamic_pointer_cast<Human>(target)->gold += (rand() % 901) + 50;
	}
}